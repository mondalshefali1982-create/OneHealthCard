...
const JWT_SECRET_KEY = process.env.JWT_SECRET || 'onehealthcard_secret_key_2026';
...
const generateUniqueHealthId = async (): Promise<string> => {
  ...
  return healthId || `ARG-${Math.floor(100000 + Math.random() * 900000)}`;
};
...
export const register = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Create corresponding profile in MongoDB
    if (role === 'patient') {
    ...
    // Generate JWT token
    const token = jwt.sign({ id: savedUser._id, role: savedUser.role }, JWT_SECRET_KEY, {
      expiresIn: '30d'
    });
