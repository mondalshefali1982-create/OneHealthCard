...
export const getPatientDashboard = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Auto-create patient profile if missing (resilience guarantee)
    if (!patientProfile) {
    ...
    const prescriptions = await Prescription.find({ patient: patientProfile._id }).sort({ createdAt: -1, date: -1 }).limit(10);
...
export const getPatientPrescriptions = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ message: `Failed to fetch prescriptions: ${error.message}` });
  }
};
