    ...
    <div className="min-h-screen bg-[#070b14] flex flex-col md:flex-row relative text-left">
      ...
      <aside className="w-full md:w-64 bg-slate-900/90 border-r border-white/10 p-6 flex flex-col justify-between shrink-0 glass">
        <div className="flex flex-col gap-8">
          ...
          <nav className="flex flex-col gap-1.5">
            ...
            <Link to="/doctor-finder" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-cyan-500/15 border border-cyan-500/30 text-cyan-400 text-sm font-bold shadow-md">
      ...
      <main className="flex-1 p-6 md:p-8 max-w-7xl mx-auto w-full overflow-y-auto">
        <div className="flex flex-col gap-8">
          ...
          <div>
            <span className="text-xs uppercase font-extrabold tracking-widest text-cyan-400 font-mono">REAL GOOGLE MAPS DIRECTORY</span>
            <h1 className="text-3xl font-extrabold text-white mt-1 mb-0">Doctor & Clinic Finder</h1>
            <p className="text-slate-400 text-sm mt-1">Discover verified specialists and real Google Maps clinics matching your exact city pins.</p>
          </div>

          {/* Search Inputs */}
          <div className="grid md:grid-cols-3 gap-4 items-center">
            <div className="relative">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4.5 w-4.5 text-cyan-400" />
              <input
                type="text"
            ...
            <div className="relative">
              <MapPin className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4.5 w-4.5 text-emerald-400" />
              <input
                type="text"
                ...
                onKeyDown={(e) => e.key === 'Enter' && handleApplySearch()}
                className="w-full pl-11 pr-4 py-3 rounded-xl glass-input text-sm text-white font-extrabold"
              />
            </div>
            ...
            <button
              ...
              className="py-3 px-6 bg-gradient-to-r from-cyan-500 to-emerald-400 text-slate-950 font-extrabold rounded-xl hover:opacity-95 transition-all text-sm flex items-center justify-center gap-2 active:scale-95 shadow-lg"
            >
              {loadingSearch ? <span className="w-5 h-5 border-2 border-slate-950 border-t-transparent rounded-full animate-spin" /> : 'Search Area'}
            </button>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 flex flex-col gap-4">
              {displayedDoctors.length === 0 ? (
                <div className="p-12 text-center rounded-3xl glass-card border border-white/10">
                  ...
                  <p className="text-slate-400 text-xs">Try searching Kolkata, Krishnagar, Ranaghat, Mumbai, or Delhi.</p>
                </div>
              ) : (
                displayedDoctors.map((doc, idx) => (
                  <div
                    key={doc._id || idx}
                    className="p-5 rounded-2xl glass-card border border-white/15 hover:border-cyan-500/40 transition-all text-left flex gap-4 items-start relative overflow-hidden shadow-2xl bg-slate-900/90"
                  >
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-cyan-500/20 to-emerald-500/20 flex items-center justify-center border border-cyan-500/30 shrink-0 mt-1">
                    ...
                    <div className="flex-1 flex flex-col gap-1.5 text-xs">
                      <div className="flex justify-between items-start">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            <h3 className="text-base font-extrabold text-white m-0 leading-snug">{doc.name}</h3>
                          </div>
                          <div className="flex items-center gap-2 mt-0.5">
                            {doc.isRegistered ? (
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 text-[10px] font-extrabold uppercase">
                                <CheckCircle className="h-3 w-3" /> Verified OneHealthCard Doctor
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-800 border border-white/10 text-slate-300 text-[10px] font-semibold">
                                <Globe className="h-3 w-3 text-cyan-400" /> Google Maps Verified Place
                              </span>
                            )}
                            {doc.openHours && (
                              <span className="text-[10px] text-emerald-400 font-bold">{doc.openHours}</span>
                            )}
                          </div>
                        </div>

                        <div className="flex flex-col items-end shrink-0">
                          <div className="flex items-center gap-1 text-amber-300 bg-amber-400/15 px-2.5 py-1 rounded-lg border border-amber-400/30">
                            <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
                            <span className="font-extrabold text-sm">{doc.rating}</span>
                          </div>
                          {doc.reviewsCount && (
                            <span className="text-[9px] text-slate-400 mt-1 font-mono">({doc.reviewsCount.toLocaleString()} reviews)</span>
                          )}
                        </div>
                      </div>

                      <span className="text-cyan-400 font-extrabold text-xs mt-0.5">{doc.specialization}</span>
                      
                      <p className="text-slate-300 flex items-start gap-1.5 mt-1 mb-0.5 font-medium">
                      ...
                      <div className="flex flex-wrap gap-2 mt-3 border-t border-white/10 pt-3">
                        <a
                          ...
                          className="px-3.5 py-2 rounded-xl bg-slate-800 text-white font-bold flex items-center gap-1.5 hover:bg-slate-700 transition-colors"
                        >
                          <Phone className="h-3.5 w-3.5 text-emerald-400" /> {doc.phoneNumber}
                        </a>
                        <button
                          onClick={() => setSelectedMapDoc(doc)}
                          className="px-3.5 py-2 rounded-xl bg-cyan-500/15 border border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/25 font-bold flex items-center gap-1.5 transition-colors"
                        >
            ...
            <div className="lg:col-span-5 relative hidden lg:block sticky top-6">
              <div className="rounded-3xl border border-white/15 glass-card overflow-hidden shadow-2xl bg-slate-900/90">
                <div className="p-4 bg-slate-950/90 border-b border-white/10 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-emerald-400 font-mono font-bold bg-emerald-400/15 px-2.5 py-0.5 rounded-full border border-emerald-400/30">
                    {cityInput || 'Kolkata'}
                  </span>
                </div>
                ...
                <div className="p-3 bg-slate-950/90 text-center text-[10px] text-slate-400 border-t border-white/10">
        ...
        {selectedMapDoc && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 bg-slate-950/80 backdrop-blur-md">
            <motion.div
              ...
              className="w-full max-w-3xl rounded-3xl glass-card border border-white/15 overflow-hidden shadow-2xl text-left relative flex flex-col max-h-[90vh] bg-slate-900/95"
            >
              {/* Modal Header */}
              <div className="p-5 bg-slate-950 border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-extrabold text-white m-0 leading-tight">{selectedMapDoc.name}</h3>
                    <p className="text-xs text-cyan-400 font-bold m-0">{selectedMapDoc.specialization}</p>
                  </div>
                </div>
                <button
              ...
              <div className="w-full h-[360px] bg-slate-950 relative border-b border-white/10">
                <iframe
              ...
              <div className="p-5 bg-slate-950 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-xs">
                ...
                <a
                  ...
                  className="px-5 py-3 bg-gradient-to-r from-cyan-500 to-emerald-400 text-slate-950 font-extrabold rounded-xl flex items-center gap-2 transition-all shadow-lg active:scale-95 text-xs whitespace-nowrap"
                >
