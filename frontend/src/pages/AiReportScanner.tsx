...
import {
  FileText, UploadCloud, Cpu, Sparkles, CheckCircle2, AlertTriangle, XCircle,
  Clock, ShieldAlert, CreditCard, Search, Settings, LogOut, Activity, ArrowRight, Eye, RefreshCw
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#070b14] flex flex-col md:flex-row relative text-left">
      ...
      <aside className="w-full md:w-64 bg-slate-900/90 border-r border-white/10 p-6 flex flex-col justify-between shrink-0 glass">
        <div className="flex flex-col gap-8">
          ...
          <nav className="flex flex-col gap-1.5">
            ...
            <Link to="/report-scanner" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-cyan-500/15 border border-cyan-500/30 text-cyan-400 text-sm font-bold shadow-md">
      ...
      <main className="flex-1 p-6 md:p-8 max-w-7xl mx-auto w-full overflow-y-auto">
        <div className="flex flex-col gap-8">
          ...
          <div>
            <span className="text-xs uppercase font-extrabold tracking-widest text-cyan-400 font-mono">OPENROUTER OCR AI SCANNER</span>
            <h1 className="text-3xl font-extrabold text-white mt-1 mb-0">AI Medical Report Analyzer</h1>
            <p className="text-slate-400 text-sm mt-1">Upload any laboratory test report image (CBC, Blood Sugar, Lipid Profile) to extract instant parameters.</p>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-6 flex flex-col gap-6">
              ...
              <div
                ...
                onDrop={handleDrop}
                className={`rounded-3xl border-2 border-dashed p-8 text-center glass-card relative overflow-hidden transition-all flex flex-col items-center justify-center min-h-[300px] ${
                  previewUrl ? 'border-cyan-500/60 bg-slate-900/90' : 'border-white/20 hover:border-cyan-400/50'
                }`}
              >
                ...
                {previewUrl ? (
                  <div className="relative w-full h-[240px] flex items-center justify-center">
                    <img src={previewUrl} alt="Report Preview" className="max-h-full max-w-full rounded-xl object-contain shadow-xl" />
                    <button
                      onClick={() => { setSelectedFile(null); setPreviewUrl(null); setAnalysisResult(null); }}
                      className="absolute top-2 right-2 p-2 bg-slate-950/80 text-rose-400 rounded-full hover:bg-slate-900 border border-white/10"
                    >
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-white mb-1">Drag & Drop Your Medical Report Image</h3>
                      <p className="text-xs text-slate-400 max-w-xs">Supports PNG, JPG, JPEG medical lab certificates and test sheets (Max 5MB).</p>
                    </div>
                    <label className="mt-2 px-5 py-2.5 bg-cyan-500 text-slate-950 font-extrabold rounded-xl text-xs hover:bg-cyan-400 cursor-pointer shadow-lg active:scale-95 transition-all">
                      Browse File
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className={`w-full py-3.5 rounded-2xl font-extrabold text-sm flex items-center justify-center gap-2 transition-all shadow-xl ${
                    previewUrl && !analyzing
                      ? 'bg-gradient-to-r from-cyan-500 via-primary-500 to-emerald-400 text-slate-950 hover:opacity-95 cursor-pointer'
                ...
                <div className="flex items-center justify-between border-t border-white/10 pt-4 mt-1">
                  <span className="text-xs text-slate-400 font-bold">Try Sample Report:</span>
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleLoadSampleReport('blood')}
                      className="px-3.5 py-1.5 rounded-xl bg-slate-800 border border-white/10 hover:border-cyan-500/40 text-cyan-300 text-xs font-semibold transition-all flex items-center gap-1.5"
                    >
                      <Sparkles className="h-3.5 w-3.5 text-cyan-400" /> Sample CBC Test
                    </button>
                    <button
                      onClick={() => handleLoadSampleReport('diabetes')}
                      className="px-3.5 py-1.5 rounded-xl bg-slate-800 border border-white/10 hover:border-amber-500/40 text-amber-300 text-xs font-semibold transition-all flex items-center gap-1.5"
                    >
                      <Sparkles className="h-3.5 w-3.5 text-amber-400" /> Sample Diabetes Test
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-6 flex flex-col gap-6">
              <div className="rounded-3xl glass-card border border-white/15 p-6 md:p-8 min-h-[420px] flex flex-col justify-between shadow-2xl bg-slate-900/90 text-left">
                
                <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
                  <div className="flex items-center gap-2">
                    <Sparkles className="h-5 w-5 text-cyan-400 animate-pulse" />
                    <h3 className="text-base font-bold text-white m-0">AI Breakdown Summary</h3>
                  </div>
                  <span className="text-[10px] uppercase font-mono font-bold text-emerald-400 bg-emerald-400/10 px-2.5 py-1 rounded-full border border-emerald-400/20">
                    REAL-TIME OCR
                  </span>
                </div>
                ...
                {analyzing ? (
                  <div className="my-auto text-center py-16 flex flex-col items-center justify-center gap-3">
                    <span className="w-10 h-10 border-4 border-cyan-400 border-t-transparent rounded-full animate-spin" />
                    <p className="text-sm font-bold text-white">OpenRouter AI Reading Lab Parameters...</p>
                    <p className="text-xs text-slate-400">Extracting Hemoglobin, Glucose, and Cholesterol stats.</p>
                  </div>
                ) : analysisResult ? (
                  <div className="flex flex-col gap-6">
                    {/* Summary Callout */}
                    <div className="p-4 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 text-xs text-slate-200 leading-relaxed">
                      <strong className="text-cyan-300 block mb-1 font-bold">Executive Summary:</strong>
                      {analysisResult.summary}
                    </div>

                    {/* Parameter Breakdown Cards */}
                    <div className="flex flex-col gap-3">
                      <span className="text-xs font-bold uppercase tracking-wider text-slate-400 font-mono">EXTRACTED TEST PARAMETERS</span>
                      {analysisResult.parameters?.map((param: any, idx: number) => (
                        <div key={idx} className="p-3.5 rounded-xl bg-slate-950/80 border border-white/10 flex justify-between items-center text-xs">
                          <div className="flex flex-col">
                            <span className="font-bold text-white">{param.name}</span>
                            <span className="text-[10px] text-slate-400">Normal Range: {param.range}</span>
                          </div>

                          <div className="flex items-center gap-3">
                            <span className="font-extrabold text-white text-sm">{param.value}</span>
                            {param.status === 'Normal' ? (
                              ...
                              <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-400 font-bold text-[10px]">
                                Attention
                              </span>
                            )}
                          </div>
                    ...
                    {analysisResult.recommendation && (
                      <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-300">
                        <strong className="block mb-1 font-bold">AI Clinical Advice:</strong>
                        {analysisResult.recommendation}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="my-auto text-center py-16 text-slate-400 flex flex-col items-center">
                    ...
                    <p className="text-white font-bold text-base mb-1">Ready for Medical Analysis</p>
                    <p className="text-xs text-slate-400 max-w-xs">Upload a report image on the left or click one of the sample buttons to see live AI extraction.</p>
                  </div>
                )}
