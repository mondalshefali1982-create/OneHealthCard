...
const Auth: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!token || !user) {
        showToast('Authentication failed. Invalid response from server.', 'error');
        return;
      }
      ...
      const msg = error.response?.data?.message || error.response?.data?.error || 'Authentication failed. Please try again.';
      showToast(msg, 'error');
    } finally {
      setLoading(false);
    }
