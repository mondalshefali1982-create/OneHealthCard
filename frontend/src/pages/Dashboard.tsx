...
export const Dashboard: React.FC = () => {
  ...
  const fetchTimeline = async () => {
    ...
    // 3. No fake hardcoded records — display empty timeline state so real doctor updates stand out cleanly!
    setPrescriptions([]);
    setLoadingTimeline(false);
  };
  ...
  useEffect(() => {
    fetchTimeline();
    // Auto refresh timeline every 5 seconds for instant doctor updates
            ...
            <div className="md:col-span-4 flex justify-center">
              <div className="relative rounded-2xl overflow-hidden shadow-lg border-2 border-white w-full h-[140px] bg-sky-600 flex items-center justify-center p-4 text-white text-center">
                <div className="flex flex-col items-center gap-2">
                  <UserCheck className="h-10 w-10 text-white animate-bounce" />
                  <span className="text-xs font-extrabold tracking-wide uppercase">Verified Doctor Consultation</span>
                  <span className="text-[10px] bg-white/20 px-2.5 py-0.5 rounded-full font-mono">LIVE SYNC ACTIVE</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 flex flex-col gap-4">
              <div className="flex justify-between items-center">
                ...
                <button onClick={fetchTimeline} className="text-sky-600 hover:text-sky-800 p-1.5 rounded-lg bg-sky-50 border border-sky-200 transition-all font-bold text-xs flex items-center gap-1">
                  <RotateCw className="h-3.5 w-3.5" /> Refresh Live
                </button>
              </div>
                ...
                {loadingTimeline ? (
                  ...
                  <div className="my-auto text-center py-12 text-slate-500">
                    ...
                    <p className="text-xs text-slate-500 max-w-xs mx-auto">
                      When a doctor updates your health card in Doctor Portal, their exact name, diagnosis, and prescribed medications will appear here live.
                    </p>
                  </div>
                ) : (
