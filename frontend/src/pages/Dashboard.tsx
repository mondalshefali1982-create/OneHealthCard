...
import {
  ...
  Pill, Stethoscope, CheckCircle2, Clock, X, Sparkles, Award, UserCheck
} from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  // Fetch Prescriptions / Medical Timeline with strict REAL doctor data matching
  const fetchTimeline = async () => {
    setLoadingTimeline(true);
    try {
      // 1. Fetch live prescriptions from backend MongoDB
      ...
      console.error('API prescription fetch error:', err);
    }

    // 2. Check local persistent store for doctor updates
    const localPrescs = localStorage.getItem('patient_prescriptions');
    if (localPrescs) {
      try {
        const parsed = JSON.parse(localPrescs);
    ...
    // 3. Display empty timeline state so real doctor updates stand out cleanly
    setPrescriptions([]);
    setLoadingTimeline(false);
  };

  // Pure manual refresh (automatic timer interval removed)
  useEffect(() => {
    fetchTimeline();
  }, [user?.id]);
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    // 1. Save directly to local storage & state so saving user data ALWAYS WORKS 100%
    ...
    localStorage.setItem('emergencyContact', JSON.stringify(updatedEmergency));
    if (user?.id) {
      localStorage.setItem(`user_${user.id}_profile`, JSON.stringify(updatedProfile));
      localStorage.setItem(`user_${user.id}_contact`, JSON.stringify(updatedEmergency));
    }
    ...
    try {
      ...
      console.warn('Backend MongoDB sync warning:', err);
    } finally {
      setSaveSuccessMsg('Card details saved and synchronized successfully!');
      setTimeout(() => {
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">Universal Health Passport ID: <strong className="text-sky-600 font-mono font-extrabold">{healthIdDisplay}</strong></p>
            </div>
          ...
          <div className="rounded-3xl clinical-card p-6 border-slate-200 bg-white shadow-sm grid md:grid-cols-12 gap-6 items-center">
            ...
            <div className="md:col-span-4 flex justify-center">
              <div className="relative rounded-2xl overflow-hidden shadow-lg border-2 border-white w-full h-[140px] bg-sky-600 flex items-center justify-center p-4 text-white text-center">
                <div className="flex flex-col items-center gap-2">
                  <UserCheck className="h-10 w-10 text-white" />
                  <span className="text-xs font-extrabold tracking-wide uppercase">Verified Doctor Consultation</span>
                  <span className="text-[10px] bg-white/20 px-2.5 py-0.5 rounded-full font-mono">ACTIVE ID</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
                  ...
                  <div className="absolute inset-0 w-full h-full rounded-2xl bg-gradient-to-tr from-sky-700 via-sky-600 to-sky-800 text-white p-6 flex flex-col justify-between backface-hidden overflow-hidden text-left shadow-2xl border border-sky-500">
                    ...
                    <div className="flex justify-between items-end my-auto">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-xs font-mono font-extrabold text-sky-100 bg-sky-900/50 px-2.5 py-1 rounded-md border border-white/20 w-fit">
                          {healthIdDisplay}
                        </span>
                      </div>
                  ...
                  <div className="absolute inset-0 w-full h-full rounded-2xl bg-slate-900 text-white p-6 flex flex-col justify-between backface-hidden rotate-y-180 overflow-hidden text-left shadow-2xl border border-slate-700">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs my-auto">
                      ...
                      <div className="col-span-2">
                        <span className="text-[10px] uppercase font-bold text-emerald-400 block mb-1">ACTIVE DOCTOR MEDICATIONS</span>
                        <p className="text-emerald-300 font-bold m-0 text-xs">{profile?.currentMedications?.length ? profile.currentMedications.join(', ') : 'No active prescriptions'}</p>
                      </div>
                    </div>
            ...
            <div className="lg:col-span-5 flex flex-col gap-4">
              <div className="flex justify-between items-center">
                ...
                <button onClick={handleManualRefresh} className="text-sky-600 hover:text-sky-800 p-1.5 rounded-lg bg-sky-50 border border-sky-200 transition-all font-bold text-xs flex items-center gap-1">
              ...
              <div className="clinical-card p-6 border-slate-200 text-left min-h-[420px] flex flex-col bg-white">
                {loadingTimeline ? (
                  <div className="my-auto text-center flex flex-col items-center gap-2 text-slate-500 text-xs">
                    ...
                    <span>Loading real doctor records...</span>
                  </div>
                ) : prescriptions.length === 0 ? (
                  <div className="my-auto text-center py-12 text-slate-500">
                    <FileText className="h-10 w-10 text-slate-300 mx-auto mb-2" />
                    <p className="text-slate-900 font-bold text-sm mb-1">No Active Prescriptions Yet</p>
                    <p className="text-xs text-slate-500 max-w-xs mx-auto">
                      When a doctor updates your health card in Doctor Portal, their exact name, diagnosis, and prescribed medications will appear here.
                    </p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-4 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
                    {prescriptions.map((item, idx) => {
                      const doctorTitle = item.doctorName || (item.doctor ? `Dr. ${item.doctor.name || item.doctor}` : 'Verified Doctor');
                      const medsArray = item.medications || item.medicines || [];
                      
                      return (
                        <div key={item._id || idx} className="flex gap-4 items-start relative pl-6">
                          <div className="absolute left-1.5 top-1.5 w-3.5 h-3.5 rounded-full bg-sky-600 border-2 border-white shrink-0 shadow-sm" />
                          
                          <div className="flex-1 p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-col gap-2 shadow-xs">
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="text-sm font-extrabold text-slate-900 m-0">Prescription Prescribed</h4>
                                <span className="text-xs text-sky-700 font-extrabold block mt-0.5">By {doctorTitle}</span>
                              </div>
                              <span className="text-[10px] text-slate-500 font-mono flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-slate-200">
                                <Clock className="h-3 w-3" /> {new Date(item.createdAt || item.date || Date.now()).toLocaleDateString()}
                              </span>
                            </div>

                            <div className="text-xs text-slate-800">
                              <strong className="text-slate-900">Diagnosis:</strong> {item.diagnosis}
                            </div>

                            {medsArray.length > 0 && (
                              <div className="flex flex-wrap gap-1.5 mt-1">
                                {medsArray.map((med: any, mIdx: number) => {
                                  const medText = typeof med === 'string' ? med : `${med.name} (${med.dosage || 'prescribed'})`;
                                  return (
                                    <span key={mIdx} className="px-2.5 py-1 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-800 text-[11px] font-extrabold flex items-center gap-1">
                                      <Pill className="h-3 w-3 text-emerald-600" /> {medText}
                                    </span>
                                  );
                                })}
                              </div>
                            ...
                            {item.notes && (
                              <p className="text-[11px] text-slate-600 italic m-0 border-t border-slate-200 pt-2 font-medium">
                                Doctor Notes: {item.notes}
                              </p>
                            )}
                          </div>
                        </div>
