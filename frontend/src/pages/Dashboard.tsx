...
import {
  ...
  Pill, Stethoscope, CheckCircle2, Clock, X, Phone
} from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  // Real Scannable QR Code URL containing Patient Health ID
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(profile?.healthId || user?.id || 'OHC-PATIENT-ID')}`;
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">Universal Health Passport ID: <strong className="text-sky-600 font-mono">OHC-{user?.id?.slice(-6)?.toUpperCase()}</strong></p>
            </div>
                  ...
                  <div className="absolute inset-0 w-full h-full rounded-2xl bg-gradient-to-tr from-sky-700 via-sky-600 to-sky-800 text-white p-6 flex flex-col justify-between backface-hidden overflow-hidden text-left shadow-2xl border border-sky-500">
                    ...
                    <div className="flex justify-between items-end my-auto">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-xs font-mono font-bold text-sky-200">ARG-{user?.id?.slice(-6)?.toUpperCase()}</span>
                      </div>
            ...
            <div className="lg:col-span-5 flex flex-col gap-4">
              ...
              <div className="clinical-card p-6 border-slate-200 text-left min-h-[420px] flex flex-col bg-white">
                {loadingTimeline ? (
                  ...
                  <div className="flex flex-col gap-6 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
                            ...
                            <div className="flex flex-wrap gap-1.5 mt-1">
                              {item.medicines.map((med: any, mIdx: number) => (
                                <span key={mIdx} className="px-2.5 py-1 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-800 text-[11px] font-bold flex items-center gap-1">
                                  <Pill className="h-3 w-3 text-emerald-600" /> {med.name} ({med.dosage})
                                </span>
                              ))}
                            </div>
                          )}
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      className="py-2.5 px-3 rounded-xl clinical-input text-xs font-bold"
                    >
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className="py-2.5 px-3 rounded-xl clinical-input text-xs font-medium"
                    />
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 py-2 px-3 rounded-xl clinical-input text-xs"
                    />
